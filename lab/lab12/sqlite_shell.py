...
class StdIOProxy(object):
    ...
    def __init__(self, stdin, stdout, stderr, codec, allow_set_code_page):
        ...
        for stream in streams:
            ...
            assert not isinstance(stream, io.RawIOBase), (
...
def main(
    ...
    def exec_script(db, filename, ignore_io_errors):
        try:
            with (
                io.open(filename, "r", encoding=codec.name) as f
            ):  # Assume .sql files are text -- any binary data inside them should be X'' encoded, not embedded directly
