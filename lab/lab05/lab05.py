...
from collections.abc import Iterator  # "t: Iterator[int]" means t is an iterator that yields integers
...
def balanced(t):
    ...
    # if is_leaf(t):
    #     return True
    # elif all(sum_tree(b) == sum_tree(branches(t)[0]) for b in branches(t)) \
    ...
    return is_leaf(t) or \
        (all(sum_tree(b) == sum_tree(branches(t)[0]) for b in branches(t))) \
        and all(balanced(b) for b in branches(t))
